...
// Base URL for event service. Prefer Vite env VITE_API_URL for flexibility in dev.
// If VITE_API_URL is provided as a host (e.g. http://localhost:8000), append the
// /api/events path. Fallback to the temporary dev backend on 8001.
...
const eventService = {
  ...
  createEvent: async (eventData) => {
    try {
      ...
      console.log('EventService - Sending data to API:', eventData);
      ...
      console.error('EventService - API error:', error.response?.data || error.message);
