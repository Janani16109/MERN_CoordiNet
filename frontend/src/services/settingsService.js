...
const getUserSettings = async () => {
  ...
  if (!token) {
    ...
    console.warn('Skipping getUserSettings: no token');
    throw err;
  }
...
const updateUserSettings = async (settings) => {
  ...
  if (!token) {
    const err = new Error('No auth token available');
    err.isAuthMissing = true;
    console.warn('Skipping updateUserSettings: no token');
    throw err;
  }
...
const getSystemSettings = async () => {
  ...
  if (!token) {
    const err = new Error('No auth token available');
    err.isAuthMissing = true;
    console.warn('Skipping getSystemSettings: no token');
    throw err;
  }
...
const updateSystemSettings = async (settings) => {
  ...
  if (!token) {
    const err = new Error('No auth token available');
    err.isAuthMissing = true;
    console.warn('Skipping updateSystemSettings: no token');
    throw err;
  }
