...
const UpdateScoreForm = ({ eventId, onScoreUpdated }) => {
  ...
  useEffect(() => {
    const fetchEventParticipants = async () => {
      if (!eventId) return;
      // Prevent unauthenticated calls (avoid 401s)
      const token = localStorage.getItem('token');
      if (!token) {
        console.warn('Skipping participants fetch: no auth token');
        return;
      }
      // Only admins/organizers should be able to update scores and fetch participants
      if (!hasPermission()) {
        console.warn('Skipping participants fetch: insufficient permissions');
        return;
      }
  ...
  if (!hasPermission()) {
    return (
      <Alert className="glass-soft" severity="warning" sx={{ color: 'rgba(255,255,255,0.95)' }}>
  ...
  return (
    <Paper elevation={0} className="glass-soft" sx={{ p: 3, borderRadius: 2 }}>
      ...
      {error && (
        <Alert className="glass-soft" severity="error" sx={{ mb: 2, color: 'rgba(255,255,255,0.95)' }}>
          {error}
        </Alert>
      )}

      {success && (
        <Alert className="glass-soft" severity="success" sx={{ mb: 2, color: 'rgba(255,255,255,0.95)' }}>
          {success}
        </Alert>
      )}
      ...
      {loading ? (
        ...
        <Box component="form" onSubmit={handleSubmit}>
          <FormControl fullWidth margin="normal">
            <InputLabel id="participant-select-label" sx={{ color: 'rgba(255,255,255,0.8)' }}>Participant</InputLabel>
            <Select
          ...
          <FormControl fullWidth margin="normal">
            <InputLabel id="achievements-label" sx={{ color: 'rgba(255,255,255,0.8)' }}>Achievements</InputLabel>
            <Select
              ...
              renderValue={(selected) => (
                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                  {selected.map((value) => (
                    <Chip key={value} label={value} sx={{ bgcolor: 'rgba(255,255,255,0.03)', color: 'rgba(255,255,255,0.92)' }} />
                  ))}
                </Box>
              )}
              MenuProps={MenuProps}
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} PaperProps={{ className: 'glass-soft' }}>
