...
exports.createEvent = async (req, res, next) => {
  try {
  const { title, description, date, location, capacity, tags, image, imageData } = req.body;
    ...
    if (req.body.organizerName) {
      ...
      // If available in the user object
      organizerName = `${req.user.firstName} ${req.user.lastName}`;
    }
    
    // Prefer imageData.url (from Cloudinary upload) if provided; otherwise use image
    const imageUrl = (imageData && imageData.url) ? imageData.url : (typeof image === 'string' ? image : (image || ''));

    console.log('createEvent - received image url:', imageUrl);
    ...
    const event = await Event.create({
      ...
      price: req.body.price !== undefined ? Number(req.body.price) : 0, // price in paise
      tags: tags || [],
      createdBy: userId,
      organizerName,
      image: imageUrl || ''
...
exports.getEventParticipants = async (req, res, next) => {
  try {
    ...
    const isAuthorized = 
      ...
      event.isUserRegistered(req.user._id);
