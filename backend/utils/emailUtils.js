...
const createTransporter = async () => {
  try {
    if (process.env.NODE_ENV === 'production') {
      ...
      // Use Ethereal credentials for development
...
exports.sendPasswordResetEmail = async (options) => {
  try {
    ...
    const mailOptions = {
      from: '"Coordinet Support" <support@coordinet.com>',
      to: options.email,
          ...
          <table style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 10px; box-shadow: 0 5px 15px rgba(0,0,0,0.05); overflow: hidden;">
            <tr>
              <td style="padding: 30px; text-align: center; background: #3b82f6;">
                <h1 style="color: white; font-size: 24px; margin: 0;">Coordinet</h1>
            ...
            <tr>
              <td style="padding: 20px; background-color: #f1f5f9; text-align: center;">
                <p style="font-size: 12px; color: #94a3b8;">Coordinet • College Event Management System</p>
              </td>
            </tr>
          </table>
        </div>
...
exports.sendEmail = async ({ to, subject, text, html }) => {
  try {
    ...
    // Do not throw to avoid bubbling email failures to user flows
    return null;
  }
};